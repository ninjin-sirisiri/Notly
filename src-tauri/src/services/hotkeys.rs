...
impl HotkeyService {
  pub fn get_all_hotkeys(conn: &Connection) -> Result<Vec<Hotkey>> {
    let mut stmt =
      conn.prepare("SELECT id, action, shortcut, enabled, created_at, updated_at FROM hotkeys")?;
  ...
  pub fn get_hotkey_by_action(conn: &Connection, action: &str) -> Result<Option<Hotkey>> {
    let mut stmt = conn.prepare(
      "SELECT id, action, shortcut, enabled, created_at, updated_at FROM hotkeys WHERE action = ?",
    )?;
